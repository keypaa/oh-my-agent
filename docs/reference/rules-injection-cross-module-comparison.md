...
- **omo rules-injector** — opencode plugin path (`packages/omo-opencode/src/hooks/rules-injector`, repo `code-yeongyu/oh-my-agent`, branch `dev`).
...
- **omo** — `~/.bun/install/global/node_modules/oh-my-agent` is a symlink to the local workspace, so `bun run build` immediately publishes the rebuilt `dist/`. Verified via `grep -c transcriptHydration dist/index.js` → 6.
