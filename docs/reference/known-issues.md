...
- **Status**: Open. Tracked at https://github.com/code-yeongyu/oh-my-agent/issues/4184.
...
## #4225 — Custom LSP config in `.opencode/oh-my-agent.jsonc` is silently ignored

- **Affects**: v4.2.3+ after the LSP to MCP migration.
- **Symptom**: Custom LSP server configuration in your project's `oh-my-agent.jsonc` is not applied at runtime.
- **Workaround**: Configure your LSP server through OpenCode's native `lsp` config instead.
- **Status**: Open. Tracked at https://github.com/code-yeongyu/oh-my-agent/issues/4225.
...
- **Status**: Open. Tracked at https://github.com/code-yeongyu/oh-my-agent/issues/4990.

## #4863 — OpenCode 1.16.x starts with only build/plan agents after install

- **Affects**: OpenCode 1.16.x with oh-my-agent 4.7.x.
- **Symptom**: After installing oh-my-agent, the OpenCode agent list only shows the built-in build/plan agents. `bunx oh-my-agent doctor` can still report `System OK`, so this looks like a successful install even though the OMO agents are not visible.
- **Workaround**: Stop OpenCode, clear the OpenCode and OMO cache directories, then reinstall:

  ```sh
  rm -rf ~/.cache/opencode/ ~/.cache/oh-my-agent/ ~/.cache/oh-my-agent/
  bunx oh-my-agent install
  ```

- **Status**: Open. Tracked at https://github.com/code-yeongyu/oh-my-agent/issues/4863.
...
- **Status**: Open. Tracked at https://github.com/code-yeongyu/oh-my-agent/issues/4710.
...
- **Symptom**: `opencode --pure` starts, but normal `opencode` clears the terminal and stalls after `service=plugin path=oh-my-agent@latest loading plugin`.
- **Workaround**: If the hang happens before `/tmp/oh-my-agent.log` gets a plugin entry, avoid the npm resolver path by using an absolute `file://` plugin path or by pre-populating the OpenCode package cache. If logs point to a malformed or locked `opencode.db`, back up and remove `~/.local/share/opencode/opencode.db*`; OpenCode recreates it on next start, but local session history is lost.
- **Status**: Open. The npm resolver timeout belongs upstream in OpenCode; tracked at https://github.com/code-yeongyu/oh-my-agent/issues/5050.
...
- **Status**: Open. Tracked at https://github.com/code-yeongyu/oh-my-agent/issues/5260.

## #5120: Sisyphus can loop on simple tasks

- **Affects**: OpenCode 1.17.0 with oh-my-agent 4.8.1.
...
- **Status**: Open. Tracked at https://github.com/code-yeongyu/oh-my-agent/issues/5120.
...
- **Symptom**: `/tmp/oh-my-agent.log` repeats `promptAsync reservation release skipped for different source` while child subagents emit message events.
- **Workaround**: If you are not using Ralph Loop in that workspace, add `"disabled_hooks": ["ralph-loop"]` to `oh-my-agent.jsonc`. If a loop is already active, run `/cancel-ralph` before disabling the hook.
- **Status**: Open. Tracked at https://github.com/code-yeongyu/oh-my-agent/issues/5105.

## #5025 — OpenCode Desktop loads the plugin but only shows native modes

- **Affects**: OpenCode Desktop on Windows with `oh-my-agent@4.7.5`.
- **Symptom**: The Desktop plugin list shows `oh-my-agent` as loaded, but the UI only exposes the native `build` and `plan` modes. The OpenCode log may include `Runtime skill source server requires Bun.serve failed to load plugin`.
...
- **Status**: Open. Tracked at https://github.com/code-yeongyu/oh-my-agent/issues/5025.
...
- **Status**: Open. Tracked at https://github.com/code-yeongyu/oh-my-agent/issues/5021.

## #3303 - Windows OpenCode proxy install can fail before OMO loads

- **Affects**: Windows OpenCode installs behind an HTTP(S) proxy, especially first startup paths that ask OpenCode to fetch `oh-my-agent@latest`.
...
- **Workaround**: Launch OpenCode from a shell that has `HTTP_PROXY` and `HTTPS_PROXY` set, then preinstall the package into OpenCode's Windows config prefix with `npm install oh-my-agent@latest --prefix "%APPDATA%\\opencode"`. Restart OpenCode and verify with `bunx oh-my-agent doctor --json`.
- **Status**: Open. Tracked at https://github.com/code-yeongyu/oh-my-agent/issues/3303.

## #4702 - Windows TUI plugin install can pause startup on a Bun npm git error

- **Affects**: Windows OpenCode startup when `tui.json` includes `oh-my-agent/tui`.
...
- **Workaround**: Remove `oh-my-agent/tui` from the `plugin` list in `tui.json` until the Bun npm install path is fixed.
- **Status**: Open. Tracked at https://github.com/code-yeongyu/oh-my-agent/issues/4702.
...
- **Status**: Open. Tracked at https://github.com/code-yeongyu/oh-my-agent/issues/4170.
...
- **Symptom**: Desktop hides Sisyphus, Hephaestus, Prometheus, Atlas, or other OMO agents even though `oh-my-agent doctor` passes.
- **First check**: Inspect the OpenCode Desktop log for `Failed to load plugin oh-my-agent@latest` and missing files under `~/.cache/opencode/packages/oh-my-agent@latest/node_modules`.
- **Cache workaround**: Close Desktop, remove the `oh-my-agent@latest` package cache, then reinstall the plugin from the same working directory with `opencode plugin oh-my-agent@latest`.
...
- **Status**: Open. Tracked at https://github.com/code-yeongyu/oh-my-agent/issues/3835 and https://github.com/code-yeongyu/oh-my-agent/issues/3456. This entry documents current triage steps; it does not resolve Desktop GUI rendering regressions.
...
- **Workaround**: In user-controlled project files such as `AGENTS.md`, prefer `oh-my-agent`, `OMO`, or `OpenCode` wording instead of the lowercase literal `opencode` when targeting Anthropic subscription providers.
- **Status**: Open. Tracked at https://github.com/code-yeongyu/oh-my-agent/issues/3435. The runtime prompt-identity cleanup still needs maintainer direction, so this workaround does not close the underlying issue.
