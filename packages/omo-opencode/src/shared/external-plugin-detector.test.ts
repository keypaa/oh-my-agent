...
describe("external-plugin-detector", () => {
  ...
  describe("detectExternalNotificationPlugin", () => {
    ...
    test("should return detected=false when only oh-my-agent is configured", () => {
      // given - opencode.json with only oh-my-agent
      const opencodeDir = path.join(tempDir, ".opencode")
      fs.mkdirSync(opencodeDir, { recursive: true })
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["oh-my-agent"] })
      )
      ...
      expect(result.detected).toBe(false)
      expect(result.pluginName).toBeNull()
      expect(result.allPlugins).toContain("oh-my-agent")
    })
    ...
    test("should detect opencode-notifier plugin", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["oh-my-agent", "opencode-notifier"] })
      )
    ...
    test("should detect opencode-notifier with version suffix", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["oh-my-agent", "opencode-notifier@1.2.3"] })
      )
    ...
    test("should detect @mohak34/opencode-notifier", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["oh-my-agent", "@mohak34/opencode-notifier"] })
      )
  ...
  describe("detectExternalSkillPlugin", () => {
    ...
    test("should detect opencode-skills plugin", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["oh-my-agent", "opencode-skills"] })
      )
    ...
    test("should detect opencode-skills with version suffix", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["oh-my-agent", "opencode-skills@1.2.3"] })
      )
  ...
  describe("detectDuplicateOmoPlugin", () => {
    test("#given a source plugin and active profile package alias #when detecting duplicates #then it reports the self-conflict", async () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({ plugin: ["oh-my-agent@latest"] }),
      )
      ...
      // then
      expect(result.detected).toBe(true)
      expect(result.pluginName).toBe("oh-my-agent")
    ...
    test("#given both package names from the rename window #when detecting duplicates #then it treats them as the same OMO plugin", async () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["oh-my-agent", "npm:oh-my-agent@latest"] }),
      )
      const nodeOs = await import("node:os")
      mock.module("node:os", () => ({
        ...nodeOs,
      ...
      // then
      expect(result.detected).toBe(true)
      expect(result.duplicatePlugins).toEqual(["oh-my-agent", "npm:oh-my-agent@latest"])
    })
  })
  ...
  describe("getDuplicateOmoPluginWarning", () => {
    test("#given duplicate OMO entries #when generating a warning #then it tells the user startup is disabled", () => {
      ...
      expect(warning).toContain("oh-my-agent@latest")
    })
  })
})
