...
| **Transform** | `create-transform-hooks.ts` | 3 | 5 | `experimental.chat.messages.transform` (+2: `team-mode-status-injector`, `team-mailbox-injector`) |
| **Continuation** | `create-continuation-hooks.ts` | 7 | 7 | Boulder/atlas/compaction/notification/audit-loop |
...
Total exposed hooks: **53 base, 63 with team-mode** (counts the 4 team-session-events handlers individually).
...
| `codegraphBootstrap` | session.created | Bootstrap codegraph MCP if configured |
| `astGrepSgProvision` | session.created | Provision ast-grep `sg` binary if needed |
...
| `auditLoopHook` | event | Multi-verifier audit loop (the-auditor + cold-eyes) |
...
├── (53 index.ts hook dirs incl. `task-reminder/` unwired — see tier tables above)
...
- **`atlasHook` vs `auditLoopHook`:** atlas handles boulder/ralph/subagent sessions; auditLoopHook runs the multi-verifier audit loop. Both fire on event but check session type first.
