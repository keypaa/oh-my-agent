...
describe("findPackageJsonUp", () => {
  ...
  it("finds a package.json whose name is the canonical oh-my-agent", () => {
    const pkgPath = join(workdir, "package.json")
    writeFileSync(pkgPath, JSON.stringify({ name: "oh-my-agent", version: "3.16.0" }))
  ...
  it("finds a package.json whose name is the aliased oh-my-agent (GH-3257)", () => {
    // A user who installed `oh-my-agent` from npm gets a node_modules entry
    // whose package.json has `name: "oh-my-agent"`. The auto-update-checker
  ...
  it("#given the interim @oh-my-opencode/omo-opencode manifest between the file and an accepted root #when walking up #then skips it and returns the accepted manifest", () => {
    ...
    writeFileSync(join(workdir, "package.json"), JSON.stringify({ name: "oh-my-agent", version: "9.9.9" }))
