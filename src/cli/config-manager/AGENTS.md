...
| File | Purpose |
|------|---------|
| `add-plugin-to-opencode-config.ts` | Register `oh-my-agent` in `.opencode/opencode.json` plugin array |
...
| `generate-omo-config.ts` | Generate `oh-my-agent.jsonc` from install selections |
...
| `plugin-name-with-version.ts` | Resolve `oh-my-agent@X.Y.Z` for installation |
| `write-omo-config.ts` | Write generated config to `.opencode/oh-my-agent.jsonc` |
